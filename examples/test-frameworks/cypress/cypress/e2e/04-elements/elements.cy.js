 ...
 * Elements Testing
 * 
 * Тести для пошуку селекторів та аналізу DOM структури
 */
...
describe('Elements Testing on Real Website', () => {
    ...
    it('should verify element structure', () => {
        cy.get('h1').then(($h1) => {
            expect($h1[0].tagName).to.eq('H1');
            expect($h1[0].textContent).to.include('Example Domain');
        });
    });
});
